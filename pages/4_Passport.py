...
# Adjust this import based on how your backend folder is structured
import db 

# UI/UX: Set page layout to wide and add a browser tab icon
st.set_page_config(page_title="Livestock Passport", page_icon="🐄", layout="wide")

# UI/UX: Custom header with styling
st.markdown("""
    <div style='background-color: #1E3A8A; padding: 15px; border-radius: 10px; margin-bottom: 25px;'>
        <h1 style='color: white; margin: 0; text-align: center;'>🐄 Official Livestock Passport</h1>
    </div>
    ...
    if selected_cow_id:
        ...
        with st.container(border=True):
            ...
            with col1:
                ...
                st.image(cow_data.get('muzzle_photo_path'), caption=f"Verified Muzzle: {selected_cow_id}", use_container_width=True)
            ...
            with col3:
                ...
                qr.make(fit=True)
                img_qr = qr.make_image(fill_color="#1E3A8A", back_color="white") # UI/UX: Brand color QR!
